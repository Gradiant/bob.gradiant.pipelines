...
class XgboostRegressorProcessor(Processor):
    """
      This class encapsulates xgboost regressor wrapper class on top of sklearn.
    ...
    def __init__(self, name='xgboost_regressor',
                 keys_correspondences=DEFAULT_KEYS_CORRESPONDENCES,
                 **kwargs):
        super(XgboostRegressorProcessor, self).__init__(name)
        self._kwargs = kwargs
        self._model = XGBRegressor(**self._kwargs)
    ...
    def run(self, x):
        ...
        x[scores_key] = self._model.predict(x[features_key]).astype(np.float64)
    ...
    def __str__(self):
        description = {
            'type': 'XgboostRegressorProcessor',
            'name': self.name,
            'kwargs': self._kwargs,
        }
