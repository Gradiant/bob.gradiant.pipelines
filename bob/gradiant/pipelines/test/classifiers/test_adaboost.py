...
from bob.gradiant.pipelines import AdaboostProcessor
...
class UnitTestAdaBoost(unittest.TestCase):
    ...
    h5f = h5py.File(base_path + '/ada_test.h5', 'w')
    ...
    def test_fit_calls_sklearn_fit(self):
        ...
        np.testing.assert_array_equal(AdaBoostClassifier.fit.call_args[0][0], self.X['features'])
        np.testing.assert_array_equal(AdaBoostClassifier.fit.call_args[0][1], expected_labels)

    @patch('sklearn.ensemble.AdaBoostClassifier.decision_function', MagicMock(return_value=scores))
    def test_transform_calls_sklearn_transform(self):
        ...
        np.testing.assert_array_equal(AdaBoostClassifier.decision_function.call_args[0][0], features)
    ...
    def test_save_model(self):
        ada = AdaboostProcessor(name='TestAda')

        ada.save(self.base_path)

        h5py.File.assert_called_once_with(os.path.join(self.base_path, 'processors/TestAda.h5'), 'w')
    ...
    def test_load_model(self):
        ada = AdaboostProcessor(name='TestAda')
        ada.load(self.base_path)

        h5py.File.assert_called_once_with(os.path.join(self.base_path, 'processors/TestAda.h5'), 'r')
