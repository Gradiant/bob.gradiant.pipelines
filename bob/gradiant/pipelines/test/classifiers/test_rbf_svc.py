...
class UnitTestRbfSvc(unittest.TestCase):
    ...
    base_path = os.path.join(TestUtils.get_result_path(), 'rbf_svc_tests')

    if not os.path.isdir(base_path):
        os.makedirs(base_path)

    h5f = h5py.File(base_path + '/rbf_svc_test.h5', 'w')
    ...
    def test_fit_calls_sklearn_fit(self):
        ...
        np.testing.assert_array_equal(SVC.fit.call_args[0][0], self.X['features'])
        np.testing.assert_array_equal(SVC.fit.call_args[0][1], expected_labels)

    @patch('sklearn.svm.SVC.decision_function', MagicMock(return_value=scores))
    def test_transform_calls_sklearn_transform(self):
        ...
        np.testing.assert_array_equal(SVC.decision_function.call_args[0][0], features)
