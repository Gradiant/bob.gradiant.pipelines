...
import unittest
import numpy as np
from bob.gradiant.pipelines import MaxScoreFusion


class UnitTestMaxScoreFusion(unittest.TestCase):
    ...
    def test_constructor_run(self):
        max_score_fusion = MaxScoreFusion()

        Y = max_score_fusion.run(self.X)

        np.testing.assert_almost_equal(Y['scores'], np.array([0.2, 0.4, 0.8]))
